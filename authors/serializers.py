...
from utility.factory import slugify
...
class E_Commerce_Serializer(serializers.ModelSerializer):  # noqa
    ...
    tag_links = serializers.HyperlinkedRelatedField(
        many=True,
        source='tags',
        view_name='store:tag_rest',  # tem que passar o lookup_field, caso contrario não encontrará o campo # noqa
...
class Dashboard_Serializer(serializers.ModelSerializer):
    ...
    category_name = serializers.StringRelatedField(read_only=True, source='category')
    category_pk = serializers.PrimaryKeyRelatedField(source='category', queryset=E_Category.objects.all(), required=False)
    ...
    def validate_title(self, data):  # VALIDAÇÃO GLOBAL | GLOBAL VALIDATION                  !IMPORTANT
        ...
        title = data
        exists = E_Commerce.objects.filter(title__iexact=title).exists()
