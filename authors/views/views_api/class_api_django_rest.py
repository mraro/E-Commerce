...
# from store.views import OurPagination
...
class Full_CRUD_API_v2(ModelViewSet):
    ...
    GenericViewSet
    """
    queryset = E_Commerce.objects.all()  # if it has some parameter send by request he will use
    ...
    def list(request):  # NOT USE GET (OR ANY METHOD HTTP), OVERWRITE WHAT HE DOES IN THIS CLASS
        print(request.user)
        remedios = E_Commerce.objects.filter(is_published=False, author=request.user)
...
class Api_CRUD_v2(RetrieveUpdateDestroyAPIView):
    queryset = E_Commerce.objects.all()
    serializer_class = serializers.Dashboard_Serializer
    pagination_class = OurPagination
"""
